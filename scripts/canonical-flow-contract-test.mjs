...
try {
  ...
  assert.deepEqual(presets.filter(p => p.supported && p.availableForNewResearch !== false).map(p => p.id), ['obj-three-factor-screening', 'obj-banking-moat'])
  ...
  assert.equal(failed.attempts.at(-1).failedStep, 1)
  assert.equal(failed.pillars[0].status, 'failed')
  assert.ok(failed.pillars[0].reason)
  ...
  assert.match(sessionView, /Lima tahap screening/)
  assert.match(sessionView, /Data dummy, salinan kedaluwarsa, dan sumber pengganti tidak digunakan/)
