...
const rawOrigins = computed(() => providerSources.value.length ? providerSources.value.map(s => s.origin) : store.screeningFunnel.flatMap(stage => stage.sourceOrigins || []))
const origins = computed(() => [...new Set(rawOrigins.value.map(sourceOriginLabel))].join(', '))
...
const demo = computed(() => rawOrigins.value.includes('demo-fixture'))
</script>
<template>
  <aside aria-label="Asal dan periode data" class="rounded-xl border border-slate-200 bg-slate-50 p-4 text-xs leading-6 text-slate-600">
    ...
    <p v-if="demo" role="alert" class="mt-2 rounded-lg bg-rose-50 p-3 text-rose-950">Data demonstrasi terdeteksi pada arsip lama. Hasil ini tidak boleh digunakan untuk keputusan investasi.</p>
