...
const bank = computed(() => props.candidates.some(candidate => candidate.formulaVersion === 'bank-screen-2f-v1'))
const metrics = computed<Record<MetricKey, string>>(() => bank.value
  ? { roePercent: 'ROE (%)', pbvRatio: 'P/BV (x)', qualityScore: 'Skor heuristik bank (/100)' } as Record<MetricKey, string>
...
<template>
  <section class="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
    ...
    <p data-testid="metric-explanation" class="text-sm leading-6 text-slate-600">{{ bank ? 'Skor bank hanya memakai ROE 60% dan P/BV 40%.' : 'Skor hanya memakai ROE, D/E, dan P/E. FCF yield menggunakan FCF FY dan kapitalisasi snapshot, bukan faktor skor.' }} Skor bukan confidence atau peluang untung.</p>
