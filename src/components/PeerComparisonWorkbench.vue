...
type MetricKey = 'roePercent' | 'debtToEquity' | 'peRatio' | 'freeCashFlowYieldPercent' | 'pbvRatio' | 'qualityScore'
const metric = ref<MetricKey>('roePercent')
const bank = computed(() => props.candidates.some(candidate => candidate.formulaVersion === 'bank-evidence-v1'))
const metrics = computed<Record<MetricKey, string>>(() => bank.value
  ? { roePercent: 'ROE provider-derived-unverified (%)', pbvRatio: 'P/BV historis provider (x)' } as Record<MetricKey, string>
...
const value = (candidate: CandidateCompany) => candidate[metric.value as keyof CandidateCompany]
...
<template>
  <section class="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
    ...
    <p data-testid="metric-explanation" class="text-sm leading-6 text-slate-600"><template v-if="bank">Kandidat ditampilkan dalam urutan market cap provider karena data top bank tersedia. Nilai ROE dan P/BV tidak menentukan keanggotaan. ROE provider-derived-unverified dan basis tanggal serta saham P/BV historis provider belum terverifikasi.</template><template v-else>Skor hanya memakai ROE, D/E, dan P/E. FCF yield menggunakan FCF FY dan kapitalisasi snapshot, bukan faktor skor. Skor bukan confidence atau peluang untung.</template></p>
