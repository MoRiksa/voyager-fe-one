...
const metric = ref<'roePercent' | 'debtToEquity' | 'peRatio' | 'freeCashFlowYieldPercent' | 'qualityScore'>('roePercent')
const metrics = { roePercent: 'ROE (%)', debtToEquity: 'Debt/Equity (x)', peRatio: 'P/E (x)', freeCashFlowYieldPercent: 'FCF yield (%)', qualityScore: 'Skor tiga faktor (/100)' }
const direction = ref<'asc' | 'desc'>('desc')
const sorted = computed(() => [...props.candidates].sort((a, b) => {
  const av = a[metric.value], bv = b[metric.value]
...
<template>
  <section class="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
    ...
    <p data-testid="metric-explanation" class="text-sm leading-6 text-slate-600">Skor hanya memakai ROE, D/E, dan P/E; bukan confidence atau peluang untung. FCF yield menggunakan FCF FY dan kapitalisasi snapshot, bukan faktor skor.</p>
