...
const bank = computed(() => activeCandidate.value?.formulaVersion === 'bank-filter-v1')
...
<template>
  <div v-if="activeCandidate" class="space-y-5">
    ...
    <section v-if="bank" class="rounded-2xl border border-slate-200 bg-white p-6"><h3 class="text-lg font-bold">Metrik filter bank</h3><p class="mt-2 text-sm leading-6 text-slate-600">bank-filter-v1 hanya memeriksa kelayakan. Kandidat lolos filter ROE/PBV dan tetap dalam urutan market cap provider.</p><dl class="mt-5 grid gap-4 sm:grid-cols-3"><div class="rounded-xl bg-slate-50 p-4"><dt class="text-sm">ROE provider-derived-unverified</dt><dd class="mt-2 font-mono font-bold">{{ metric(activeCandidate.roePercent, '%') }}</dd></div><div class="rounded-xl bg-slate-50 p-4"><dt class="text-sm">P/BV historis provider</dt><dd class="mt-2 font-mono font-bold">{{ metric(activeCandidate.pbvRatio, 'x') }}</dd></div><div class="rounded-xl bg-slate-50 p-4"><dt class="text-sm">Common FY</dt><dd class="mt-2 font-mono font-bold">{{ activeCandidate.financialPeriod || 'Tidak tersedia' }}</dd></div></dl><p class="mt-4 rounded-xl bg-amber-50 p-4 text-sm leading-6 text-amber-950">Basis earnings, equity, dan tanggal dapat tidak cocok. Filter bukan validasi kualitas atau kesehatan bank; lakukan uji tuntas independen.</p></section>
