...
<template>
  <div v-if="activeCandidate" class="space-y-5">
    ...
    <section v-if="bank" class="rounded-2xl border border-slate-200 bg-white p-6"><h3 class="text-lg font-bold">Bukti data bank</h3><p class="mt-2 text-sm leading-6 text-slate-600">bank-evidence-v1 menampilkan data top bank yang tersedia dalam urutan market cap provider. Nilai ROE dan P/BV tidak menentukan keanggotaan.</p><dl class="mt-5 grid gap-4 sm:grid-cols-3"><div class="rounded-xl bg-slate-50 p-4"><dt class="text-sm">ROE provider-derived-unverified</dt><dd class="mt-2 font-mono font-bold">{{ metric(activeCandidate.roePercent, '%') }}</dd></div><div class="rounded-xl bg-slate-50 p-4"><dt class="text-sm">P/BV historis provider, basis belum terverifikasi</dt><dd class="mt-2 font-mono font-bold">{{ metric(activeCandidate.pbvRatio, 'x') }}</dd></div><div class="rounded-xl bg-slate-50 p-4"><dt class="text-sm">Common FY</dt><dd class="mt-2 font-mono font-bold">{{ activeCandidate.financialPeriod || 'Tidak tersedia' }}</dd></div></dl><p class="mt-4 rounded-xl bg-amber-50 p-4 text-sm leading-6 text-amber-950">Basis earnings dan equity dapat tidak cocok; basis tanggal dan saham P/BV belum terverifikasi.</p></section>
