...
<template>
  <div class="mx-auto max-w-5xl space-y-7 px-4 py-7 sm:px-6 sm:py-10 lg:px-8">
    <header class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      <p class="mt-3 max-w-3xl text-sm leading-6 text-slate-600">Ikuti keputusan penyaringan dalam bahasa ringkas. Setiap event mempertahankan jenis sumber dan referensi artifact yang tersedia.</p>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7" aria-label="Timeline aktivitas">
      <div v-if="!store.toolCalls.length" class="flex flex-col items-center justify-center py-12 text-center">
        ...
        <h2 class="mt-4 text-lg font-bold text-slate-900">Belum ada aktivitas</h2>
        <p class="mt-2 max-w-sm text-sm text-slate-500">Aktivitas riset akan muncul di sini setelah proses dimulai. Kembali ke sesi untuk memulai atau melanjutkan riset.</p>
