...
<template>
  <div class="page-shell space-y-8">
    <header class="rounded-3xl bg-[#102138] p-7 text-white sm:p-10"><p class="text-xs font-semibold text-blue-200">Workspace riset finansial</p><h1 class="mt-4 max-w-3xl text-3xl font-bold tracking-tight sm:text-4xl">Temukan kandidat untuk diteliti lebih lanjut</h1><p class="mt-5 max-w-2xl text-sm leading-7 text-slate-300">Pilih screening generic tiga faktor atau kesehatan bank IDX. Setiap objective memakai kriteria dan formula terpisah.</p><router-link to="/research/new" class="button-primary mt-6">Susun riset</router-link><p class="mt-4 max-w-2xl text-xs leading-6 text-slate-300">Screening bank terbatas pada ROE, capital-to-RWA proxy, dan P/BV; bukan NPL, NIM, moat, kualitas kredit, atau CAR regulator resmi. Skor bukan peluang untung.</p></header>
    ...
    <section class="max-w-3xl"><h2 class="text-lg font-bold">Baca alasan, risiko, dan periode sebelum skor</h2><p class="mt-3 text-sm leading-7 text-slate-600">Hasil menampilkan alasan seleksi, data yang belum lengkap, serta sumber dan waktu snapshot. Verifikasi laporan sumber secara independen sebelum menggunakan shortlist.</p><router-link to="/methodology" class="text-link mt-3 inline-block">Pelajari cara penilaian</router-link></section>
  </div>
</template>
