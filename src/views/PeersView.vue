...
const isBank = computed(() => comparisonCandidates.value.some(candidate => candidate.formulaVersion === 'bank-evidence-v1'))
  ...
  <div class="page-shell space-y-7">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm">
      ...
      <p class="text-sm text-slate-600 mt-1 max-w-3xl">
         Tinjau nilai absolut kandidat akhir. {{ isBank ? 'Tinjauan bank menampilkan ROE provider-derived-unverified dan P/BV historis provider dalam urutan market cap provider.' : 'Pertumbuhan belum tersedia.' }} Benchmark sektor independen belum tersedia.
      </p>
    </div>
    ...
    <section data-testid="peers-next" class="grid gap-5 rounded-2xl border border-[#407EC9]/20 bg-[#407EC9]/5 p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-center">
      <div><p class="section-kicker">Lanjutkan riset</p><h2 class="mt-2 text-xl font-bold text-slate-950">{{ comparisonCandidates.length >= 2 ? 'Baca laporan data dan keterbatasannya' : 'Tinjau kembali hasil seleksi' }}</h2><p class="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{{ comparisonCandidates.length >= 2 ? isBank ? 'Laporan merangkum urutan market cap provider, ketersediaan data, sumber, risiko basis, dan batas analisis.' : 'Laporan merangkum ranking, alasan pemilihan, risiko, sumber data, dan batas analisis untuk seluruh kandidat.' : 'Perbandingan membutuhkan sedikitnya dua kandidat. Lihat tahap seleksi untuk memahami hasil sesi ini.' }}</p></div>
