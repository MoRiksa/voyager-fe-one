...
const isBank = computed(() => store.candidates.some(candidate => candidate.formulaVersion === 'bank-health-3f-v1'))
  ...
  <div class="report-page page-shell space-y-6">
    ...
    <template v-else>
      ...
      <section v-show="documentMode || activeSection === 'summary'" id="report-panel-summary" :role="documentMode ? undefined : 'tabpanel'" aria-labelledby="report-tab-summary" class="space-y-5 rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <h3 class="font-bold">Langkah riset berikutnya</h3><p class="text-sm leading-6 text-slate-600">Periksa laporan keuangan sumber dan tanggal harga. <template v-if="isBank">Shortlist ini belum memeriksa NPL, NIM, moat, kualitas kredit, atau CAR regulator resmi.</template><template v-else>Verifikasi risiko serta kecocokan sektor sebelum menggunakan shortlist; kesehatan bank dan keberlanjutan dividen belum dijawab.</template></p>
      </section>
