...
import { formatDateTime, objectiveStatusLabel, sourceOriginLabel } from '../utils/presentation'
...
const isRestricted = computed(() => store.isActiveSessionRestricted)
const objectiveLabel = computed(() => objectiveStatusLabel(store.report.objectiveStatus))
const isBank = computed(() => store.report.contract?.objectiveType === 'bank-evidence' || store.candidates.some(candidate => candidate.formulaVersion === 'bank-evidence-v1'))
...
const download = async (format: 'markdown' | 'json') => {
  if (exportBusy.value || isRestricted.value) return
...
<template>
  <div class="report-page page-shell space-y-6">
    ...
    <template v-else>
      <div v-if="isRestricted" data-testid="restricted-report" class="rounded-2xl border-2 border-rose-300 bg-rose-50 p-5 text-sm leading-6 text-rose-950"><strong class="block text-base">Laporan ini tidak dapat digunakan untuk keputusan investasi.</strong> Metode lama tidak sesuai dengan tujuan tertulis atau data wajib tidak lengkap. Nama perusahaan, urutan, dan skor di bawah hanya dipertahankan sebagai arsip dan tidak boleh dianggap sebagai pilihan saham. Penyalinan, cetak, ekspor, dan penggunaan ulang dinonaktifkan.</div>
      <header class="flex flex-wrap items-start justify-between gap-5"><div><p class="section-kicker">{{ isRestricted ? 'Arsip laporan' : 'Laporan riset' }}</p><h1 data-testid="objective-status" class="mt-2 max-w-3xl text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">{{ isRestricted ? 'Hasil tidak dapat digunakan' : objectiveLabel }}</h1><p class="mt-3 text-sm text-slate-600">Proses: {{ sessionStatusMeta(store.status, store.isExecuting).label }} · Laporan dibuat: <time :datetime="store.report.timestamp">{{ formatDateTime(store.report.timestamp) }}</time></p></div><div v-if="!isRestricted" class="flex flex-wrap gap-2 print:hidden"><button type="button" class="button-primary" @click="print">Cetak / simpan PDF</button><details><summary class="button-secondary cursor-pointer">Pilihan ekspor</summary><div class="mt-2 grid gap-2"><button type="button" :disabled="exportBusy" class="button-secondary" @click="download('markdown')">Unduh Markdown</button><button type="button" :disabled="exportBusy" class="button-secondary" @click="download('json')">Unduh JSON</button></div></details></div></header>
      <p v-if="exportError" role="alert" class="text-sm text-rose-700">{{ exportError }}</p>
      <div v-if="!isRestricted" class="print:hidden"><button type="button" class="button-secondary" @click="copySummary">Salin ringkasan</button><p role="status" class="mt-2 text-sm">{{ copyStatus }}</p></div>
      <p class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">{{ store.report.contract?.coveragePolicy || 'Cakupan laporan lama belum terverifikasi.' }} <template v-if="isBank">Data bank yang tersedia ditampilkan; nilai ROE dan P/BV tidak menentukan apakah bank masuk daftar. Basis laba, ekuitas, tanggal, dan jumlah saham belum diverifikasi.</template><template v-else>Skor bukan peluang untung atau tingkat keyakinan. Bukti satu sumber belum diverifikasi independen.</template> Ringkasan otomatis tidak tersedia.</p>
      ...
      <section v-show="documentMode || activeSection === 'summary'" id="report-panel-summary" :role="documentMode ? undefined : 'tabpanel'" aria-labelledby="report-tab-summary" class="space-y-5 rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <p v-if="store.report.objectiveStatus === 'cannot_assess'" class="text-sm text-amber-900">{{ isBank ? 'Satu atau lebih bank pada cakupan top-eight belum memiliki data wajib, sehingga tinjauan lengkap belum dapat diberikan. Ini bukan kegagalan nilai finansial.' : 'Sebagian perusahaan belum dapat dinilai karena data tidak lengkap. Ini berbeda dari gagal kriteria finansial.' }}</p>
        ...
        <h3 class="font-bold">Batas analisis</h3><ul v-if="isBank" class="list-disc space-y-2 pl-5 text-sm leading-6 text-slate-600"><li>ROE memakai earnings / total equity provider dengan basis yang dapat tidak cocok.</li><li>P/BV historis provider belum terverifikasi basis tanggal dan sahamnya.</li><li>Ringkasan AI belum tersedia.</li></ul><ul v-else class="list-disc space-y-2 pl-5 text-sm leading-6 text-slate-600"><li v-for="limitation in store.report.limitations" :key="limitation">{{ limitation }}</li></ul>
        <h3 class="font-bold">Langkah riset berikutnya</h3><p class="text-sm leading-6 text-slate-600">Periksa laporan keuangan sumber dan tanggal harga. <template v-if="isBank">Data ROE dan P/BV ini memerlukan verifikasi basis serta uji tuntas independen sebelum digunakan.</template><template v-else>Verifikasi risiko serta kecocokan sektor sebelum menggunakan shortlist; analisis bank dan keberlanjutan dividen belum dijawab.</template></p>
      </section>
      <section v-show="documentMode || activeSection === 'candidates'" id="report-panel-candidates" :role="documentMode ? undefined : 'tabpanel'" :aria-labelledby="documentMode ? undefined : 'report-tab-candidates'"><h2 class="sr-only">Data kandidat</h2><template v-if="documentMode"><ReportCandidateAnalysis v-for="candidate in store.candidates" :key="candidate.symbol" :selected-ticker="candidate.symbol" :candidates="[candidate]" :session-id="store.report.sessionId" /></template><ReportCandidateAnalysis v-else v-model:selected-ticker="selectedTicker" :candidates="store.candidates" :session-id="store.report.sessionId" /><p class="mt-4 text-sm leading-6 text-slate-600">{{ isBank ? 'ROE dan P/BV ditampilkan sebagai angka, bukan nilai kelayakan. Urutan mengikuti kapitalisasi pasar dari penyedia data.' : store.report.peerComparisonNotes }}</p></section>
      <section v-show="documentMode || activeSection === 'risks'" id="report-panel-risks" :role="documentMode ? undefined : 'tabpanel'" aria-labelledby="report-tab-risks" class="space-y-4 rounded-2xl border border-slate-200 bg-white p-6"><h2 class="text-xl font-bold">Risiko dan yang belum diketahui</h2><p class="text-sm leading-6">{{ store.report.uncertaintyNotes }}</p><article v-for="candidate in store.candidates" :key="candidate.symbol"><h3 class="font-bold">{{ candidate.symbol }}</h3><ul v-if="isBank" class="mt-2 list-disc space-y-1 pl-5 text-sm leading-6"><li>Basis earnings dan total equity provider dapat tidak cocok.</li><li>Basis tanggal dan saham P/BV historis provider belum terverifikasi.</li></ul><ul v-else class="mt-2 list-disc space-y-1 pl-5 text-sm leading-6"><li v-for="risk in candidate.potentialConcerns" :key="risk">{{ risk }}</li><li v-if="!candidate.potentialConcerns.length">Risiko spesifik belum tersedia; bukan berarti tidak ada risiko.</li></ul></article><ul v-if="!isBank" class="list-disc space-y-2 pl-5 text-sm leading-6"><li v-for="limitation in store.report.limitations" :key="limitation">{{ limitation }}</li></ul></section>
      <section v-show="documentMode || activeSection === 'evidence'" id="report-panel-evidence" :role="documentMode ? undefined : 'tabpanel'" aria-labelledby="report-tab-evidence" class="space-y-5 rounded-2xl border border-slate-200 bg-white p-6"><h2 class="text-xl font-bold">Bukti dan metode</h2><p class="text-sm leading-6">{{ store.report.methodologyOverview }}</p><ul class="list-disc space-y-2 pl-5 text-sm"><li v-for="criterion in store.report.contract?.criteria || store.activePlan.criteria" :key="criterion">{{ criterion }}</li></ul><article v-for="candidate in store.candidates" :key="candidate.symbol" class="border-t border-slate-200 pt-4"><h3 class="font-bold">{{ candidate.symbol }}</h3><p class="mt-2 break-words text-xs leading-6">Sumber: {{ candidate.providerSource?.sourceRef || 'Tidak tersedia' }} · Asal: {{ sourceOriginLabel(candidate.sourceOrigin) }}<br />FY: {{ candidate.financialPeriod || 'Tidak tersedia' }} · Harga per: {{ candidate.priceAsOf || 'Tidak tersedia' }} · Diambil: {{ candidate.providerSource?.retrievedAt || 'Tidak tersedia' }}<br />Formula: {{ candidate.formulaVersion || 'Tidak tersedia' }} · Bukti: {{ candidate.evidenceStatus || 'Tidak tersedia' }} · Narasi: {{ candidate.narrativeStatus || 'Tidak tersedia' }}</p><ul class="mt-3 space-y-3 text-sm"><li v-for="citation in candidate.evidenceCitations" :key="`${citation.source}-${citation.metric}`"><strong>{{ citation.metric }}: {{ citation.value }}</strong><p>{{ citation.context }}</p><p class="break-words text-xs text-slate-500">{{ citation.source }} · {{ citation.period || 'Periode tidak tersedia' }} · {{ citation.asOf || 'Tanggal tidak tersedia' }}</p></li></ul></article></section>
      <footer class="text-xs leading-6 text-slate-600"><p>{{ store.report.disclaimer }}</p><div class="mt-4 flex flex-wrap gap-3 print:hidden"><router-link to="/research" class="button-secondary">Kembali ke pustaka</router-link><router-link :to="`/research/${store.report.sessionId}/screener`" class="button-secondary">Tinjau proses data</router-link><router-link v-if="!isRestricted" to="/research/new" data-testid="report-use-template" class="button-secondary">Gunakan sebagai template</router-link></div></footer>
    </template>
  </div>
</template>
