...
const objectiveLabel = computed(() => store.report.objectiveStatus ? { answered: 'Tujuan terjawab pada cakupan terbatas', not_answered: 'Tujuan belum terjawab: tidak ada data kandidat', cannot_assess: 'Tujuan belum dapat dinilai sepenuhnya' }[store.report.objectiveStatus] : 'Status pemenuhan tujuan tidak tersedia')
...
<template>
  <div class="report-page page-shell space-y-6">
    ...
    <template v-else>
      ...
      <p class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">{{ store.report.contract?.coveragePolicy || 'Cakupan kontrak laporan lama belum terverifikasi.' }} <template v-if="isBank">Kandidat memiliki data yang tersedia untuk ditinjau; nilai ROE dan P/BV tidak menentukan keanggotaan. ROE provider-derived-unverified dan P/BV historis provider memakai common FY; basis earnings, equity, tanggal, dan saham belum terverifikasi.</template><template v-else>Skor bukan peluang untung atau tingkat keyakinan. Bukti satu sumber belum diverifikasi independen.</template> Narasi AI tidak tersedia pada screening ini.</p>
