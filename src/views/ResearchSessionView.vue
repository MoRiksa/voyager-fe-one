...
<template>
  <div class="page-shell space-y-6">
    ...
    <section aria-labelledby="phase-title" class="rounded-2xl border border-slate-200 bg-white p-6"><h2 id="phase-title" class="text-xl font-bold">Lima tahap screening</h2><p class="mt-2 text-sm leading-6 text-slate-600">Setiap tahap berhenti jika sumber wajib gagal. Data dummy, salinan kedaluwarsa, dan sumber pengganti tidak digunakan.</p><ol class="mt-5 grid gap-3 lg:grid-cols-5"><li v-for="(pillar, index) in store.pillars" :key="pillar.id" class="rounded-xl border p-4" :class="pillar.status === 'failed' ? 'border-rose-300 bg-rose-50' : pillar.status === 'completed' ? 'border-emerald-200 bg-emerald-50' : pillar.status === 'active' ? 'border-blue-300 bg-blue-50' : 'border-slate-200 bg-slate-50'"><span class="font-mono text-xs font-bold">0{{ index + 1 }}</span><h3 class="mt-2 text-sm font-bold">{{ pillar.name }}</h3><p class="mt-1 text-xs leading-5 text-slate-600">{{ pillar.subtitle }}</p><p class="mt-3 text-xs font-semibold">{{ pillar.status === 'completed' ? 'Selesai' : pillar.status === 'active' ? 'Sedang berjalan' : pillar.status === 'failed' ? 'Gagal' : 'Menunggu' }}</p><p v-if="pillar.reason" role="alert" class="mt-2 text-xs leading-5 text-rose-800">{{ pillar.reason }}</p></li></ol></section>
